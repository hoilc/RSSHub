...
module.exports = async (ctx, next) => {
    ...
    if (ctx.state.data) {
        ...
        if (ctx.query) {
            ...
            // translate title
            if (!ctx.query.opencc && ctx.query.translateTo) {
                const titles = ctx.state.data.item.reduce((prev, curr) => prev + '\n' + curr.title.replace(new RegExp('\n', 'g'), ''), '');
                const translated_result = await ctx.cache.tryGet(`translate-cache-${md5(titles + ctx.query.translateTo)}`, async () => await translate(titles, { to: ctx.query.translateTo }).then((res) => res.text).catch(() => {}), config.cache.routeExpire);
                translated_result.split('\n').forEach((translated, index) => ctx.state.data.item[index].title = translated);
            }
        }
    }
};
